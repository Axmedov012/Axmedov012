    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form class="m-3" method="POST" action="{{ route('projects.update',$project) }}">
                    @method('PUT')
                    @csrf
                    <!-- title -->
                    <div>
                        <x-input-label for="" :value="__('Title')" />
                        <x-text-input id="" class="block mt-1 w-full" type="text" name="title" :value="$project->title" required autofocus autocomplete="title" />
                        <x-input-error :messages="$errors->get('title')" class="mt-2" />
                    </div>
                    <!-- Description -->
                    <div>
                        <x-input-label for="description" :value="__('Description')" />
                        <x-text-input id="" class="block mt-1 w-full" type="text" name="description" :value="$project->description" required autofocus autocomplete="description" />
                    ...
                    <div>
                        ...
                        <x-text-input id="" class="block mt-1 w-full" type="date" name="deadline_at" :value="$project->deadline_at" required autofocus autocomplete="deadline_at" />
                        ...
                        <select name="user_id" id="" class="block mt-1 w-full border-gray-400 focus:border-indigo-500 focus:ring-indigo-500 rounded-b">
                            @foreach($users as $user)
                                <option value="{{$user->id}}"
                                    @selected(old('user_id') == $project->user->id)> {{$user->first_name .''.$user->last_name}}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="client_id" id="" class="block mt-1 w-full border-gray-400 focus:border-indigo-500 focus:ring-indigo-500 rounded-b">
                            @foreach($clients as $client)
                                <option value="{{$client->id}}"
                                    @selected(old('client_id') == $project->client->id)> {{$client->company_name }}
                                </option>
                            @endforeach
                        </select>
